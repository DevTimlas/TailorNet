...
def get_nearest_points_and_normals(vert, base_verts, base_faces):
    ...
    nearest_tri = nearest_tri.ravel().astype(np.longlong)
    nearest_part = nearest_part.ravel().astype(np.longlong)
    ...
    cl_tri_idxs = np.nonzero(nearest_part == 0)[0].astype(int)
    cl_vrt_idxs = np.nonzero(nearest_part > 3)[0].astype(int)
    cl_edg_idxs = np.nonzero((nearest_part <= 3) & (nearest_part > 0))[0].astype(int)
